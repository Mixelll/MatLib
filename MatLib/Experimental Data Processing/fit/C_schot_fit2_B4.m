function [str, fun, span, FittedVarsCell] = C_schot_fit2_B4(x,y,A,es,N,Vb,n,a,n0,varargin)
p = inputParser;
...
elseif strcmpi(x, 'model')
    str = 'C = A*sqrt(es*e0*q*N)/sqrt(2*n*(n*(Vb-k*T)-V-a*sqrt(-V))), a = h/4/sqrt(pi)*v_fermi_gr*sqrt(es*e0*N)/sqrt(2*q*n0)'; fun = []; span = []; FittedVarsCell = [];
else
T = p.Results.T;
f = p.Results.f;
...
fun = @(V) A.*sqrt(q.*es.*e0.*N./(2.*n.*(n.*(Vb-k*T)-V-a.*sqrt(-V))));
