function [str, fun, span, FittedVarsCell] = C_schot_fit2_B2(x,y,A,es,N,Vb,n,a,n0,varargin)
p = inputParser;
...
elseif strcmpi(x, 'model')
    str = 'C = A*sqrt(es*e0*q*N)/sqrt(2*n*(n*(Vb-k*T)-V+a*(sqrt(Vb-k*T-V)-sqrt(Vb-k*T)))), a = h/4/sqrt(pi)*v_fermi_gr*sqrt(es*e0*N)/sqrt(2*q*n0)'; fun = []; span = []; FittedVarsCell = [];
else
T = p.Results.T;
f = p.Results.f;
...
cschot = A*sqef*Nsq/sqrt(2*n*(n*(Vb-k*T)-V-a*(sqrt(Vb-k*T-V)-sqrt(Vb-k*T))));
...
if any(strcmp(coefficients, 'Nsq'))
    ...
    FittedVarsCell(:, end+1) = {'N'; N; 'Doping [cm^{-3}]'};
end
...
if any(strcmp(coefficients, 'n0sq'))
    ...
    FittedVarsCell(:, end+1) = {'n0'; n0; 'Graphene Doping [cm^{-2}]'};
end
...
FittedVarsCell(:, end+1) = {'a'; a; 'a_Graphene_Ef_Shift_Param'};
str = [str ' a=' num2str(a,2)];
str = [str ' R^{2}=' num2str(gof.rsquare,3) ];
FittedVarsCell(:, end+1) = {'R2'; gof.rsquare; 'R^{2}'};
fun = @(V) A.*sqrt(q.*es.*e0.*N./(2.*n.*(n.*(Vb-k*T)-V-a.*(sqrt(Vb-k.*T-V)-sqrt(Vb-k.*T)))));
