...
cschot = A*sqef*Nsq/sqrt(2*n*(n*(Vb-k*T)-V-a*(sqrt(Vb-k*T-V-a*(sqrt(Vb-k*T-V)-sqrt(Vb-k*T)))-sqrt(Vb-k*T))));
...
fun = @(V) A.*sqrt(q.*es.*e0.*N./(2.*n.*(n.*(Vb-k*T)-V-a.*(sqrt(Vb-k.*T-V-a.*(sqrt(Vb-k.*T-V)-sqrt(Vb-k.*T)))-sqrt(Vb-k.*T)))));
