function [str, fun, span, FittedParamCell] = C_schot_fit_A_f(x,y,boxin,A,es,N,Vb,n,varargin)
dvec = ~isnan(y);
x = x(dvec);
y = y(dvec);
...
boxstr = [num2str(max(x(1),boxin(1))) ' to ' num2str(min(x(end),boxin(2)))];
if length(boxin)==4
    boxstr = ['x- ' boxstr ' y- ' num2str(max(y(1),boxin(3))) ' to ' num2str(min(y(end),boxin(4)))];
end
fo = fitoptions('Method','NonlinearLeastSquares', 'Lower',coefficients_lim(:,1), 'StartPoint',coefficients_lim(:,2), 'Upper',coefficients_lim(:,3), 'Robust','Bisquare');
